...
#else /* !CONIOEX */
{
	...
	if (GetConsoleScreenBufferInfo(h, &csbi)) {
		...
		psz = (LPTSTR)_malloca(len * sizeof(TCHAR)); if (psz == NULL) perror("メモリ不足"), exit(1);//ueda
		pw = (LPWORD)_malloca(len * sizeof(WORD)); if (pw == NULL) perror("メモリ不足"), exit(1);//ueda
...
#else /* !CONIOEX */
{
	...
	if (GetNumberOfConsoleInputEvents(h, &dwEvent) && dwEvent) {
		DWORD			dwRead = 0;
		PINPUT_RECORD	pInpRec = (PINPUT_RECORD)_malloca(dwEvent * sizeof(INPUT_RECORD));//ueda
		if (pInpRec == NULL) perror("メモリ不足"), exit(1);//ueda
		if (ReadConsoleInput(h, pInpRec, dwEvent, &dwRead) && dwRead) {
			PINPUT_RECORD	pir = pInpRec;//ueda
			DWORD			dw;
 ...
 * @retval	0	エラー
 */
int opensound(const char *path)	//ueda
#ifdef CONIOEX
;
#else /* !CONIOEX */
{
	...
	if (strstr(szMidiExt, szExt)) {
		...
		(void)lstrcpynA(psi->szPath, path, MAX_PATH);//ueda
...
// 配列表示(文字)(全部)
//=============================
void print_x_y_all(int x, int y, const char* p[], int color)
#ifdef CONIOEX
;
#else /* !CONIOEX */
{
...
// 配列表示(文字)(1つ)
//=============================
void print_x_y_one(int x, int y, const char* p, int color)
#ifdef CONIOEX
;
#else /* !CONIOEX */
{
...
// 配列表示(数字)(1つ)
//=============================
void print_x_y_one_num(int x, int y, int num, int color)
#ifdef CONIOEX
;
#else /* !CONIOEX */
{
